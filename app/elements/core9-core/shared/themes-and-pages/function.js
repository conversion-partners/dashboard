...
function updateOutput() {
  ...
  if(typeof instance.content === 'undefined') {
    saveNestable();
  } else if(JSON.stringify(instance.content) !== JSON.stringify(content)) {
    saveNestable();
  }
  //console.log(JSON.stringify(instance.content));
...
function getCurrentPageId() {
  ...
  if(typeof pageName == 'undefined') {
    pageName = $('#editor_holder2 > div > h3 > span')
      .text();
  }
...
function setActiveTab(tab) {
  $('#editor_holder2 > div > div.rows > div.tabs.list-group.col-md-2 > a.list-group-item')
    .eq(tab)[0].click();
}
  ...
  $('#nestable')
    .nestable({
      ...
      callback: function (l, e) {
        var element = $(e[0])
          .find('.dd-content')[0].childNodes[0];
...
function initTemplateSelectBoxes() {
  ...
  $(".language-data")
    .on("change", function () {
      ...
      var query = {};
      if(TYPEOFPAGE == 'pages') {
        query.domain = $(".template-data")
          .val();
      }
      if(TYPEOFPAGE == 'themes') {
        query.theme = $(".template-data")
          .val();
      }
      query.language = $(this)
        .val();
